...
function Start-Cleanup {
    <#
    if ([System.IO.File]::Exists("$Script:LogPath\14 Days Before Password Expiration.csv")) {
        "$(Get-Date) Delete Item: 14 Day" | Out-File -FilePath $script:Log -Append -Encoding ascii
        Remove-Item -Path "$Script:LogPath\14 Days Before Password Expiration.csv"
        #recheck to see if file is deleted
        if ([System.IO.File]::Exists("$Script:LogPath\14 Days Before Password Expiration.csv")) {
            "$(Get-Date) Delete Item: 14 Day Still Exists" | Out-File -FilePath $script:Log -Append -Encoding ascii
        }
        else {
            "$(Get-Date) Delete Item: 14 Day was Deleted" | Out-File -FilePath $script:Log -Append -Encoding ascii
        }
    }
    if ([System.IO.File]::Exists("$script:LogPath\7 Days Before Password Expiration.csv")) {
        "$(Get-Date) Delete Item: 7 Day" | Out-File -FilePath $script:Log -Append -Encoding ascii
        Remove-Item -Path "$Script:LogPath\7 Days Before Password Expiration.csv"
        if ([System.IO.File]::Exists("$Script:LogPath\14 Days Before Password Expiration.csv")) {
            "$(Get-Date) Delete Item: 7 Day Still Exists" | Out-File -FilePath $script:Log -Append -Encoding ascii
        }
        else {
            "$(Get-Date) Delete Item: 7 Day was Deleted" | Out-File -FilePath $script:Log -Append -Encoding ascii
        }  
    }
    if ([System.IO.File]::Exists("$script:LogPath\7 Days Before Password Expiration.csv")) {
        "$(Get-Date) Delete Item: 7 Day" | Out-File -FilePath $script:Log -Append -Encoding ascii
    ...
    if ([System.IO.File]::Exists("$script:LogPath\90 Days before Password Expiration.csv")) {
        "$(Get-Date) Delete Item: 90 Day" | Out-File -FilePath $script:Log -Append -Encoding ascii
        Remove-Item -Path "$Script:LogPath\90 Days Before Password Expiration.csv"
        if ([System.IO.File]::Exists("$Script:LogPath\14 Days Before Password Expiration.csv")) {
            "$(Get-Date) Delete Item: 90 Day Still Exists" | Out-File -FilePath $script:Log -Append -Encoding ascii
        }
        else {
            "$(Get-Date) Delete Item: 90 Day was Deleted" | Out-File -FilePath $script:Log -Append -Encoding ascii
        }  
    }
    if ([System.IO.File]::Exists("$script:LogPath\All Password Expiration.csv")) {
        "$(Get-Date) Delete Item: Expired" | Out-File -FilePath $script:Log -Append -Encoding ascii
        Remove-Item -Path "$Script:LogPath\All Password Expiration.csv"
        if ([System.IO.File]::Exists("$Script:LogPath\14 Days Before Password Expiration.csv")) {
            "$(Get-Date) Delete Item: Expired Still Exists" | Out-File -FilePath $script:Log -Append -Encoding ascii
        }
        else {
            "$(Get-Date) Delete Item: Expired was Deleted" | Out-File -FilePath $script:Log -Append -Encoding ascii
        }  
    }
    if ([System.IO.File]::Exists("$script:LogPath\Password Expired.csv")) {
        "$(Get-Date) Delete Item: Expired" | Out-File -FilePath $script:Log -Append -Encoding ascii
        Remove-Item -Path "$Script:LogPath\All Password Expiration.csv"
        if ([System.IO.File]::Exists("$Script:LogPath\14 Days Before Password Expiration.csv")) {
            "$(Get-Date) Delete Item: Expired Still Exists" | Out-File -FilePath $script:Log -Append -Encoding ascii
    ...
    if ([System.IO.File]::Exists("$Script:LogPath\Users With Passwords That Do Not Expire.csv")) {
        "$(Get-Date) Delete Item: Do Not Expire" | Out-File -FilePath $script:Log -Append -Encoding ascii
        Remove-Item -Path "$Script:LogPath\Users With Passwords That Do Not Expire.csv"
        if ([System.IO.File]::Exists("$Script:LogPath\14 Days Before Password Expiration.csv")) {
            "$(Get-Date) Delete Item: Do Not Expire Still Exists" | Out-File -FilePath $script:Log -Append -Encoding ascii
        }
        else {
            "$(Get-Date) Delete Item: Do Not Expire was Deleted" | Out-File -FilePath $script:Log -Append -Encoding ascii
        }
    }
    #>
    Get-ChildItem -Name "$script:LogPath\*csv" | Remove-Item -Recurse -Verbose | Out-File -FilePath $script:Log -Append -Encoding ascii
